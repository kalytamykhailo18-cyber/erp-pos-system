...
// Direct file upload (without Cloudinary)
exports.uploadFileDirect = async (req, res, next) => {
  const t = await sequelize.transaction();
  let tempFilePath = null;
  ...
  try {
    ...
    const tempDir = os.tmpdir();
    const tempFileName = `price-import-${uuidv4()}${fileExt}`;
    ...
    const batch = await PriceImportBatch.create({
      ...
      supplier_id: supplier_id || null,
      file_name: fileName,
      file_url: null, // No Cloudinary URL for direct upload
      file_type: fileType,
      file_size_bytes: file.size,
      status: 'PROCESSING',
      ocr_required: fileType === 'PDF',
      ocr_engine: fileType === 'PDF' ? 'pdf-parse' : null,
      margin_type: 'PERCENTAGE',
      margin_value: parseFloat(margin_percentage) || 30,
      margin_percentage: parseFloat(margin_percentage) || 30,
      rounding_rule: rounding_rule || 'NEAREST',
...
exports.getBatchItems = async (req, res, next) => {
  try {
    ...
    const { match_type, search } = req.query;
